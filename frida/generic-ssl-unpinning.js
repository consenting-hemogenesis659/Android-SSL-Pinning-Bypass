'use strict'
/*
 * Android SSL-unpinning (adapted from Eltion's Instagram-SSL-Pinning-Bypass).
 * Three parts:
 *   1. Generic (always on): Conscrypt TrustManagerImpl.checkTrustedRecursive +
 *      SSLContext.init trust-all — works on standard Android TLS apps.
 *   2. Meta "liger" native hook: proxygen::SSLVerification in libliger.so
 *      (Meta's OLDER network stack). Only fires if libliger.so is present.
 *   3. disableHTTP3(): deletes Instagram's mobileconfig to drop QUIC (disabled
 *      by default — see the commented call below).
 *
 * NOTE: modern Instagram/Facebook route through the *Tigon* stack, which this
 * script does NOT cover. For current Meta apps use ../frida/tigon_bypass.js.
 * Credit: Eltion (original "Instagram-SSL-Pinning-Bypass"; repo since removed by GitHub).
 */
